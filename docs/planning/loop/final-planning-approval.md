...
- **이해 게이트**: 사용자가 아래 3문장을 재진술할 수 있으면 빌드 진입 권장 —
  1. 기술: Next.js(App Router)+Prisma+로컬 PostgreSQL+로컬 파일 저장소, AI는 미결이라 어댑터로 격리
  2. 범위: 네이버 블로그 1채널 수동 게시 MVP — 자동 발행은 절대 안 함
  3. 최대 리스크: 홈피드 노출 보장 불가 + 글당 수익이 목표 대비 낮아 사업 가정 검증이 필요

## 8. First Recommended Phase

- **P0** (P0-T1 프로젝트 생성+로컬 PostgreSQL 스키마 마이그레이션 → P0-T2 네이버 API 클라이언트+폴백) — REQ-013이 모든 상류 데이터의 원천이고, 04 스키마가 이제 착수 가능 상태이기 때문
