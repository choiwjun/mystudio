import { productImportMaxRedirects, validateProductImportUrl } from "@/lib/security/productImport";
...
type WorkerMetadata = {
  ...
  readonly popularity_score?: unknown;
  readonly popularity_rank?: unknown;
  readonly popularity_source?: unknown;
};
...
function buildWorkerRequestBody(url: URL): Record<string, unknown> {
  return {
    ...
    max_redirects: productImportMaxRedirects,
    options: {
      max_redirects: productImportMaxRedirects,
    },
  };
  ...
  try {
    const response = await fetch(workerUrl, {
      ...
      headers: {
        ...
        "x-insane-search-max-redirects": productImportMaxRedirects.toString(),
      },
...
async function validateRedirectChain(value: unknown): Promise<InsaneSearchFailureReason | null> {
  ...
  if (value.length > productImportMaxRedirects) {
...
function metadataToProduct(
  ...
  const popularityScore = cleanNonNegativeNumber(metadata.popularity_score);
  const popularityRank = cleanPositiveInteger(metadata.popularity_rank);
  const popularitySource = cleanText(metadata.popularity_source, 80);
  ...
  const candidate: InsaneSearchProductInput = {
    ...
    ...(popularityScore === undefined ? {} : { popularity_score: popularityScore }),
    ...(popularityRank === undefined ? {} : { popularity_rank: popularityRank }),
    ...(popularitySource === undefined ? {} : { popularity_source: popularitySource }),
